# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.6.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.6", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.6", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.6. "
            "Esta version corrige el 403 Forbidden de los ejercicios interactivos en Docker: el "
            "HTML del ejercicio se guardaba como 0o600 (solo root) y el nginx del sandbox, que corre "
            "como otro usuario, no podia leerlo. Ahora se guarda como 0o644.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.6", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.6", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Corregido: 403 Forbidden en ejercicios interactivos (Docker)")
    pdf.bullet([
        "Sintoma: al abrir un ejercicio interactivo el iframe mostraba '403 Forbidden' de nginx.",
        "Causa: el HTML del ejercicio se guardaba con permisos 0o600 (herencia de tempfile.mkstemp), "
        "legible solo por el usuario de la API (root); el sandbox lo sirve como usuario nginx.",
        "Solucion: el fichero se guarda como 0o644 (contenido publico servido de forma aislada). "
        "En local no se reproducia porque el sandbox Python corria con el mismo usuario.",
    ])
    pdf.section("Remediacion de ficheros ya subidos")
    pdf.bullet([
        "Los .html subidos antes del fix conservan 0o600. Darles lectura en el host: "
        "chmod -R a+rX media/  (o docker compose exec api sh -c 'chmod -R a+rX /app/media').",
    ])
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.6.pdf")
    build_pdf(output)
