# Manual Técnico y de Usuario — Plataforma Educativa V-0.3.0
...
## 0. Novedades de V-0.3.0 (nueva funcionalidad)

Esta versión añade los **ajustes generales del sitio**: nombre editable y selección de tipografía.

- **Nombre del sitio editable** desde el panel **Apariencia y ajustes**. Se refleja al instante en
  la barra pública, el pie de página y el sidebar de administración.
- **Selector de fuente de letra** con un catálogo curado para un portal infantil/primaria:
  ...
  - **Sistema** (pila nativa, sin descarga) por defecto.
- La fuente se aplica en tiempo real a todo el sitio mediante la variable CSS `--cms-font`.
- **Fuentes self-hosted** (subset latino, pesos 400/700): las sirve la propia app, **no un CDN
  externo**, de modo que no se expone la IP de los menores a terceros (RGPD/DSA, §10). Los `.woff2`
  viven en `frontend/public/fonts/` y los descarga `frontend/scripts/download_fonts.py`.
- **Backend:** campo `fuente_activa` en el dominio, métodos `cambiar_nombre`/`cambiar_fuente` con
  validación, endpoint `PUT /api/v1/config/general` (solo admin) y migración Alembic **005**.
- **Tests:** 4 de integración nuevos (guardado, guarda de admin, fuente inválida → 400, nombre vacío
  → 400). Suite total: **78 tests**, todos en verde.
...
> **Nota de la V-0.3.0:** estos endpoints crean la fila singleton de configuración de forma
...
> **Nota (V-0.3.0):** en desarrollo, el frontend habla siempre con su mismo origen
...
> **Novedad de la V-0.3.0:** si una operación de paleta falla (por ejemplo, intentar crear una
...
> **Nota (V-0.3.0):** el fichero `tests/integration/test_configuration_endpoints.py` incluye un
