# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.10.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.10.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.10.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.10.0. "
            "Esta version anade ROBUSTEZ: copias de seguridad automaticas de la base de datos "
            "(copia en caliente del SQLite + rotacion) y purga programada de la papelera "
            "(borrado definitivo del contenido que lleva mas de 30 dias borrado), con un nuevo "
            "panel de administracion para gestionar las copias.",
            border=0, fill=True,
        )
        self.ln(5)
        self.set_font("Helvetica", "", 10)
        self.set_text_color(100, 100, 100)
        self.cell(0, 6, "Fecha: 2026-06-16", align="C", new_x="LMARGIN", new_y="NEXT")
        self.cell(0, 6, "Version del software: V-0.10.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.10.0", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: copias de seguridad automaticas de la BD")
    pdf.bullet([
        "Tarea en segundo plano (en proceso, sin broker) que copia la BD SQLite en caliente "
        "usando la online backup API (consistente y segura con WAL, a diferencia de copiar el "
        "fichero a pelo).",
        "Rotacion automatica: conserva solo las BACKUP_KEEP copias mas recientes (por defecto 7).",
        "Configurable por .env: BACKUP_ENABLED, BACKUP_DIR, BACKUP_INTERVAL_HOURS, BACKUP_KEEP.",
        "Las copias se guardan en ./data/backups, dentro del volumen de datos (persisten).",
    ])
    pdf.section("Anadido: purga programada de la papelera")
    pdf.bullet([
        "El contenido que lleva en la papelera mas de TRASH_RETENTION_DAYS (por defecto 30 dias) "
        "se elimina de forma DEFINITIVA automaticamente.",
        "El borrado sigue siendo logico primero (papelera); la purga solo actua sobre lo ya "
        "borrado, de modo que la eliminacion irreversible nunca es el primer paso.",
        "Configurable/desactivable por .env: TRASH_PURGE_ENABLED, TRASH_RETENTION_DAYS, "
        "TRASH_PURGE_INTERVAL_HOURS.",
    ])
    pdf.section("Anadido: panel admin 'Copias de seguridad'")
    pdf.bullet([
        "Nueva seccion (solo admin) que lista las copias existentes (nombre, tamano, fecha).",
        "Boton 'Crear copia ahora' para generar una copia manual bajo demanda.",
        "Endpoints: GET /api/v1/admin/backups y POST /api/v1/admin/backups (guarda de rol admin).",
    ])
    pdf.section("Notas")
    pdf.bullet([
        "Sin cambios de esquema: NO hay migracion Alembic. La antiguedad en papelera se deduce de "
        "updated_at (un contenido borrado esta congelado, asi que marca cuando entro a la papelera).",
        "123 tests en verde (12 nuevos: servicio de backup, purga y endpoints admin).",
        "Version de la API actualizada de 0.8.0 a 0.10.0.",
    ])
    ...
    pdf.section("Requisitos (produccion con HTTPS)")
    pdf.bullet([
        "Docker 24+ y Docker Compose v2",
        "Dos dominios (APP_DOMAIN y SANDBOX_DOMAIN) con registros DNS A -> IP del servidor",
        "Puertos 80 y 443 redirigidos del router al servidor (para Let's Encrypt y HTTPS)",
    ])
    pdf.section("Arranque rapido con Docker (Caddy + HTTPS)")
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.9.0.zip && cd plataforma-educativa-v0.9.0\n\n"
        ...
        "# 3. DNS: registros A de ambos dominios -> IP publica del servidor.\n"
        "#    Router: redirigir 80 y 443 al servidor.\n\n"
        "# 4. Levantar (construye las imagenes; Caddy pide los certificados)\n"
        "docker compose up -d --build\n"
        "docker compose logs -f caddy   # ver la emision de certificados\n\n"
        "# Web: https://APP_DOMAIN   (el acceso es solo por dominio+HTTPS, no por IP:puerto)\n"
        "# El entrypoint del api ejecuta: alembic upgrade head + crea admin si no hay usuarios"
    )
    pdf.section("Variables de entorno (.env, en la raiz)")
    pdf.kv_table([
        ("APP_DOMAIN", "Dominio de la web (p. ej. plataforma.tudominio.com)"),
        ("SANDBOX_DOMAIN", "Subdominio del sandbox (p. ej. sandbox.tudominio.com)"),
        ...
        ("SANDBOX_BASE_URL / APP_ORIGINS / CORS", "se derivan de los dominios en docker-compose"),
    ])
    ...
    pdf.kv_table([
        ...
        ("Mantenimiento", "backup service (rotacion), purga papelera, endpoints admin"),
        ("Total", "123 tests - todos pasan en V-0.10.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.10.0)", "Robustez: backups automaticos de la BD + purga programada de papelera."),
        ("Hecho (V-0.9.0)", "Produccion: HTTPS con Caddy (Let's Encrypt) + sandbox en subdominio."),
        ("Siguiente", "Imagen base backend sin vulns (escaner). E2E Playwright (incl. test sandbox)."),
        ("Mas adelante", "Logotipo configurable. Buscador full-text (FTS5). Contador de visitas/auditoria."),
    ])
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.10.0.pdf")
    build_pdf(output)
