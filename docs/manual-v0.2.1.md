# Manual Técnico y de Usuario — Plataforma Educativa V-0.2.1
...
## 0. Novedades de V-0.2.1 (release de corrección)

Esta versión es un **PATCH** centrado en corregir un fallo del contexto `configuration`:

- **Corregido el error 500 al aplicar o guardar paletas de color.** Cuando la fila singleton
  `site_config` aún no existía en la base de datos, el repositorio insertaba dos filas con la
  misma clave primaria, lo que provocaba `IntegrityError: UNIQUE constraint failed: site_config.id`.
  La causa de fondo es que la sesión de producción usa `autoflush=False`, por lo que `session.get()`
  no encontraba el modelo recién añadido (pendiente de *flush*) y `save()` creaba un duplicado.
  La solución unifica el acceso al singleton en `_get_or_create_model()` con un `flush()` explícito.
- **Frontend:** las mutaciones de paletas ahora propagan los errores de la API y se muestra un
  banner de error accesible en la página de Apariencia, en lugar de fallar en silencio.
- **Frontend:** el cliente de API usa `baseUrl` relativo y Vite reenvía `/api` al backend mediante
  proxy, eliminando los problemas de CORS en desarrollo.
- **Tests:** nuevo fichero de integración del contexto `configuration` (7 tests). El fixture replica
  `autoflush=False` de producción para que el caso de regresión sea válido. Suite total: **74 tests**.
...
> **Nota de la V-0.2.1:** estos endpoints crean la fila singleton de configuración de forma
...
> **Nota (V-0.2.1):** en desarrollo, el frontend habla siempre con su mismo origen
...
> **Novedad de la V-0.2.1:** si una operación de paleta falla (por ejemplo, intentar crear una
...
> **Nota (V-0.2.1):** el fichero `tests/integration/test_configuration_endpoints.py` incluye un
