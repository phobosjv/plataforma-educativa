# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.5
...
## 0. Novedades de V-0.8.5

Acceso a la web por **IP y dominio a la vez**.

- **Corregido — Vite bloqueaba el acceso por dominio** (`Blocked request. This host ... is not
  allowed`). Vite permite las IP por defecto, pero exige declarar los dominios en
  `server.allowedHosts`.
- **Añadido — hosts permitidos configurables:** `vite.config.ts` lee `VITE_ALLOWED_HOSTS` (lista de
  hosts separados por comas, o `true` para cualquiera), propagada desde la variable `ALLOWED_HOSTS`
  del `.env` vía `docker-compose.yml`.
- **`.env.example`:** guía de acceso dual. `APP_ORIGINS` admite varios orígenes (separados por
  **espacios**) y `CORS_ALLOW_ORIGINS` varios (separados por **comas**).
- **Nota:** cambiar `vite.config.ts` requiere **reconstruir** la imagen del frontend
  (`docker compose up -d --build`).

### Versiones recientes
- **V-0.8.4** — Fix API en bucle de reinicio (PYTHONPATH) + mensajes de error de login claros.
