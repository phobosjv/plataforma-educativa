# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.7.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.7.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.7.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.7.0. "
            "Esta version hace el catalogo publico NAVEGABLE por taxonomias (curso -> asignatura -> "
            "ejercicios), pensado para ninos; anade la eliminacion definitiva desde la "
            "papelera; y muestra un boton claro para crear ejercicios interactivos.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.7.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.7.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: catalogo navegable por taxonomias (para ninos)")
    pdf.bullet([
        "El catalogo deja de ser una lista plana: navegacion guiada Curso -> Asignatura -> "
        "Ejercicios, con tarjetas grandes y coloridas y migas de pan clicables.",
        "Cursos agrupados por ciclo (con n.o de actividades); asignaturas con su color. El estado "
        "va en la URL (?curso=&asignatura=): el boton atras del navegador funciona y es compartible.",
        "Solo se muestran cursos/asignaturas con actividades (sin callejones vacios). Boton Ver todo.",
    ])
    pdf.section("Anadido: eliminacion definitiva desde la papelera")
    pdf.bullet([
        "Boton Eliminar definitivamente (con confirmacion) en la papelera. Solo admin, irreversible.",
        "Caso de uso PurgarContenidoHandler (solo purga lo que ya esta en papelera, seccion 7). "
        "Endpoint DELETE /api/v1/contenidos/{id}/purgar. Versiones borradas en cascada.",
    ])
    pdf.section("Anadido: crear ejercicios interactivos mas visible")
    pdf.bullet([
        "Dos botones en Contenidos: + Articulo de texto y + Ejercicio interactivo (antes solo uno).",
        "El formulario llega con el tipo preseleccionado (?tipo=).",
        "3 tests de integracion nuevos (purga). Suite total: 104 tests, todos en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.7.0)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.7.0.zip && cd plataforma-educativa-v0.7.0\n\n"
    ...
    pdf.body(
        "Nota V-0.7.0: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.kv_table([
        ...
        ("Total", "104 tests - todos pasan en V-0.7.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.7.0)")
    pdf.body(
    ...
    pdf.kv_table([
        ("Hecho (V-0.7.0)", "Ejercicios interactivos: subida de HTML + origen sandbox aislado."),
        ("Hecho (V-0.7.0)", "Editar taxonomia (ciclo/curso/asignatura) de un contenido existente."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.7.0.pdf")
    build_pdf(output)
