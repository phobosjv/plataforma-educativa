# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.9
...
## 0. Novedades de V-0.8.9

Robustez: **frontend de producción** servido por nginx.

- **Cambiado:** el frontend en Docker se sirve como **build estático tras nginx** (Dockerfile
  multi-stage), en vez del servidor de desarrollo de Vite (`npm run dev`), no apto para producción.
- Nuevo `nginx/frontend.conf`: sirve la SPA con *fallback* a `index.html` (React Router), **proxya
  ...
  **`client_max_body_size` a 16 MB** (evita el `413` al subir imágenes/HTML; el default de nginx es 1 MB).
- `docker-compose.yml`: `frontend` pasa a `5173:80` y deja de necesitar `VITE_API_TARGET`/
  `VITE_ALLOWED_HOSTS`. **`ALLOWED_HOSTS` ya no hace falta** (nginx sirve cualquier `Host` → IP y
  dominio funcionan solos).
- **Despliegue:** `docker compose up -d --build frontend`. El `npm run dev` para desarrollo local
  sigue igual.

### Versiones recientes
- **V-0.8.8** — Enlace "Ver la web" en el panel admin.
- **V-0.8.7** — Fix: el sandbox cacheaba los errores 1 año (`Cache-Control` sin `always`).
---
