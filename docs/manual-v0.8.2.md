# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.2
...
## 0. Novedades de V-0.8.2

Esta versión hace **navegable el path** del ejercicio maximizado.

- **Path navegable:** en la barra superior del ejercicio maximizado, cada tramo del path enlaza al
  catálogo filtrado por esa taxonomía:
...
- **Título del sitio:** a la izquierda del path, separado por un divisor, se muestra el nombre del
  sitio como **enlace al inicio** (`/`).
- **Catálogo:** nuevo soporte de `?ciclo=<id>` que filtra la pantalla de inicio a los cursos de ese
  ciclo, con su miga de pan. Navegación 100% en cliente; sin cambios de API ni backend.
