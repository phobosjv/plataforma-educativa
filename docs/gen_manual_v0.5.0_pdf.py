# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.5.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.5.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.5.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.5.0. "
            "Esta version anade los FONDOS / ESTAMPADOS tematicos del sitio: 6 tematicas (Aula, "
            "Naturaleza, Espacio, Oceano, Geometrico, Granja) mas Ninguno, como patrones SVG "
            "self-hosted recoloreados con la paleta activa, configurables desde el panel.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.5.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.5.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: fondos / estampados tematicos")
    pdf.bullet([
        "Nuevo ajuste en 'Apariencia y ajustes' para elegir un fondo tipo estampado, con 6 "
        "tematicas y la opcion Ninguno (por defecto).",
        "Tematicas: Aula (pizarra, lapiz, reloj, regla), Naturaleza (pino, hoja, seta, bellota), "
        "Espacio (cohete, planeta, estrella, luna), Oceano (pez, concha, burbujas, estrella de "
        "mar), Geometrico (formas y lunares) y Granja (granero, trigo, valla, sol).",
        "Selector con vista previa del estampado ya tenido con la paleta.",
    ])
    pdf.section("Como funciona (privacidad y legibilidad)")
    pdf.bullet([
        "Patrones SVG self-hosted (public/patterns/*.svg): los sirve la propia app, no un CDN.",
        "Se recolorean con la paleta activa mediante una mascara CSS, a baja opacidad y detras del "
        "contenido, de modo que NO afectan la legibilidad de los menores.",
    ])
    pdf.section("Backend y tests")
    pdf.bullet([
        "Dominio: campo fondo_activo y metodo cambiar_fondo con validacion (FONDOS_PERMITIDOS).",
        "Endpoint PUT /api/v1/config/general extendido. Migracion Alembic 006 (columna fondo_activo).",
        "3 tests de integracion nuevos. Suite total: 83 tests, todos en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.5.0)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.5.0.zip && cd plataforma-educativa-v0.5.0\n\n"
    ...
    pdf.body(
        "Nota V-0.5.0: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.bullet([
        ...
        "Pendiente: sanitizacion en cliente con DOMPurify (V-0.5.0).",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "83 tests - todos pasan en V-0.5.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.5.0)")
    pdf.body(
    ...
    pdf.kv_table([
        ("V-0.5.0", "Formulario completo de creacion/edicion de contenidos. "
         "Subida de HTML para ejercicios. Editor de texto enriquecido (Tiptap). "
         "Sanitizacion en cliente con DOMPurify."),
        ("V-0.5.0", "Buscador full-text (FTS5) en catalogo publico. "
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.5.0.pdf")
    build_pdf(output)
