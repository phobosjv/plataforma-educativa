# Manual Técnico y de Usuario — Plataforma Educativa V-0.10.0
...
## 0. Novedades de V-0.10.0

**Robustez:** copias de seguridad automáticas de la base de datos y purga programada de la papelera.

- **Añadido — Copias de seguridad automáticas:** una tarea en segundo plano (en proceso, sin broker)
  hace una **copia en caliente** del SQLite con la *online backup API* (consistente y segura con WAL,
  a diferencia de copiar el fichero a pelo) y **rota** las antiguas conservando solo las
  `BACKUP_KEEP` más recientes (por defecto 7). Configurable por `.env`
  (`BACKUP_ENABLED`, `BACKUP_DIR`, `BACKUP_INTERVAL_HOURS`, `BACKUP_KEEP`). Las copias se guardan en
  `./data/backups`, dentro del volumen de datos (persisten entre despliegues).
- **Añadido — Purga programada de la papelera:** el contenido que lleva en la papelera más de
  `TRASH_RETENTION_DAYS` (por defecto **30 días**) se elimina de forma **definitiva**
  automáticamente. El borrado sigue siendo lógico primero (`CLAUDE.md §7`); la purga solo actúa
  sobre lo ya borrado. Desactivable con `TRASH_PURGE_ENABLED=false` o `TRASH_RETENTION_DAYS=0`.
- **Añadido — Panel admin "Copias de seguridad"** (solo `admin`): lista las copias existentes
  (nombre, tamaño, fecha) y permite **crear una copia manual** bajo demanda. Endpoints
  `GET`/`POST /api/v1/admin/backups`.
- **Sin cambios de esquema** → no hay migración Alembic. La antigüedad en papelera se deduce de
  `updated_at` (un contenido borrado está congelado, así que marca cuándo entró a la papelera).
- La tarea de mantenimiento arranca/para con el `lifespan` de FastAPI. Versión de la API: `0.8.0` → `0.10.0`.

### Versiones recientes
- **V-0.9.0** — Producción: HTTPS automático con Caddy (Let's Encrypt) + sandbox en subdominio.
- **V-0.8.9** — Robustez: frontend de producción (build estático tras nginx).
---
