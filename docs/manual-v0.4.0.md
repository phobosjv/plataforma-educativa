# Manual Técnico y de Usuario — Plataforma Educativa V-0.4.0
...
## 0. Novedades de V-0.4.0 (nueva funcionalidad)

Esta versión añade la **creación y edición de artículos de texto** desde el panel de administración.
...
- **Editor WYSIWYG (Tiptap)**: negrita, cursiva, tachado, encabezados (H2/H3), listas, cita y enlaces.
- Botones **"+ Nuevo artículo"** y **"Editar"** en la lista de contenidos.
- **Sanitización del HTML de artículos** (CLAUDE.md §10), en dos capas:
  - **Servidor (SIEMPRE):** puerto de dominio `HtmlSanitizer` + implementación `Nh3HtmlSanitizer`
    (`nh3`), con allowlist conservadora (sin `script`/`style`/`iframe`, sin atributos `on*`, sin
    esquemas `javascript:`). Se aplica al `body_html` de tipo `texto` antes de persistir.
  - **Cliente:** segunda capa con **DOMPurify** al renderizar el artículo en la vista pública.
- **Tests:** 2 de integración de sanitización nuevos. Suite total: **80 tests**, todos en verde.

> **Alcance:** esta entrega cubre solo el tipo **texto**. Los contenidos **interactivos** (subida de
> HTML) se abordarán junto con el subdominio sandbox, requisito de seguridad para servirlos (§10). La
> clasificación se asigna al **crear** (el contrato actual no la edita en actualización).

### Versiones recientes
- **V-0.3.0** — Ajustes generales: nombre del sitio editable y selección de fuente (self-hosted).
- **V-0.2.1** — Corrección del error 500 al aplicar/guardar paletas de color.
...
| Método | Ruta | Auth | Descripción |
|---|---|---|---|
| GET | `/config/` | — | Obtener configuración (nombre, fuente activa, paleta activa + paletas) |
...
> **Nota de la V-0.4.0:** estos endpoints crean la fila singleton de configuración de forma
...
> **Nota (V-0.4.0):** en desarrollo, el frontend habla siempre con su mismo origen
...
> **Novedad de la V-0.4.0:** si una operación de paleta falla (por ejemplo, intentar crear una
...
> **Nota (V-0.4.0):** el fichero `tests/integration/test_configuration_endpoints.py` incluye un
