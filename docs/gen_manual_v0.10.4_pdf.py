# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.10.4.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.10.4", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.10.4", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.10.4. "
        ...
        self.cell(0, 6, "Version del software: V-0.10.4", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.10.4", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: suite E2E con Playwright (CLAUDE.md 11)")
    pdf.bullet([
        "Tests en navegador real contra la pila completa (API + sandbox + frontend) que la propia "
        "config levanta en puertos AISLADOS (8101/8102/5273) sobre una BD y media e2e desechables.",
        "catalogo: navegacion publica curso -> asignatura -> ejercicio -> ficha.",
        "sandbox-seguridad: AISLAMIENTO del sandbox (10): iframe allow-scripts sin allow-same-origin, "
        "origen sandbox distinto, y el JS del ejercicio NO alcanza el origen padre.",
        "ejercicio-pantalla-completa: maximizar y salir con Escape.",
        "admin-login-crud: login de admin + crear/borrar un articulo.",
    ])
    pdf.section("Soporte")
    pdf.bullet([
        "backend/scripts/seed_e2e.py: siembra determinista (admin, taxonomia, ejercicio con HTML que "
        "intenta escapar del sandbox, y un articulo).",
        "Guia docs/e2e.md y scripts npm run test:e2e / test:e2e:ui.",
        "Sin cambios de la aplicacion: 144 tests de backend + 5 E2E en verde. API -> 0.10.4.",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "123 tests - todos pasan en V-0.10.4"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.10.4)", "Robustez: backups automaticos de la BD + purga programada de papelera."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.10.4.pdf")
    build_pdf(output)
