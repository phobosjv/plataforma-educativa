# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.2.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.2", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.2", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.2. "
            "Esta version hace NAVEGABLE el path del ejercicio maximizado: cada tramo "
            "(ciclo / curso / asignatura) enlaza al catalogo filtrado por esa taxonomia, y a la "
            "izquierda se muestra el titulo del sitio como enlace al inicio.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.2", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.2", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: path navegable en el ejercicio maximizado")
    pdf.bullet([
        "En la barra superior del ejercicio maximizado, cada tramo del path es ahora un enlace al "
        "catalogo filtrado por esa taxonomia:",
        "  ciclo (p. ej. primaria) -> catalogo del ciclo (/?ciclo=<id>).",
        "  curso (p. ej. 3o) -> cursos/asignaturas de ese curso (/?curso=<id>).",
        "  asignatura -> ejercicios de esa asignatura y curso (/?curso=<id>&asignatura=<id>).",
        "  titulo -> tramo actual, sin enlace.",
        "A la izquierda del path, separado por un divisor, se muestra el TITULO DEL SITIO como "
        "enlace al inicio (/).",
    ])
    pdf.section("Catalogo: filtro por ciclo")
    pdf.bullet([
        "Nuevo soporte de ?ciclo=<id>: la pantalla de inicio del catalogo se filtra a los cursos "
        "de ese ciclo, con su miga de pan.",
        "Navegacion 100% en cliente (React Router + query params); sin cambios de API ni backend.",
        "Clases CSS todas prefijadas cms-.",
    ])
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.2)", "Path navegable en el ejercicio maximizado + titulo del sitio como enlace."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.2.pdf")
    build_pdf(output)
