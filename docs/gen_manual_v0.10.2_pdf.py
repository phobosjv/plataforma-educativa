# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.10.2.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.10.2", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.10.2", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.10.2. "
        ...
        self.cell(0, 6, "Version del software: V-0.10.2", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.10.2", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: descarga de copias de seguridad desde el navegador")
    pdf.bullet([
        "Nuevo endpoint GET /api/v1/admin/backups/{nombre} (solo admin) que sirve el fichero de la "
        "copia como adjunto.",
        "Boton 'Descargar' en cada fila del panel 'Copias de seguridad'. Usa el cliente tipado con "
        "el token de sesion (parseAs blob): el admin guarda la copia en su PC.",
    ])
    pdf.section("Seguridad")
    pdf.bullet([
        "El nombre solicitado se valida en el servicio de backup: formato exacto "
        "app-YYYYMMDD-HHMMSS.sqlite3 + comprobacion de contencion en el directorio de copias.",
        "Cualquier intento de path traversal (../, rutas absolutas) se rechaza con 404.",
    ])
    pdf.section("Notas")
    pdf.bullet([
        "Sin cambios de esquema: no hay migracion. 136 tests en verde (13 nuevos).",
        "API version -> 0.10.2.",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "123 tests - todos pasan en V-0.10.2"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.10.2)", "Robustez: backups automaticos de la BD + purga programada de papelera."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.10.2.pdf")
    build_pdf(output)
