# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.22.1.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def cover(self) -> None:
        ...
        self.multi_cell(
            INNER_W, 6,
            "Arreglo de COMPATIBILIDAD del visor de fichas PDF en moviles. Muchos navegadores "
            "moviles (iOS Safari, Android Chrome) no renderizan un PDF embebido en un <iframe> y lo "
            "dejaban en blanco. Ahora la ficha se renderiza con PDF.js dentro de un <div> (un canvas "
            "por pagina), que funciona en casi todos los navegadores. Sin cambios de esquema ni de "
            "contrato de la API.",
            border=0, fill=True,
        )
        self.ln(5)
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.chapter_title("0", "El PDF ahora se ve en movil (visor PDF.js)")
    pdf.section("Que cambia")
    pdf.bullet([
        "La ficha PDF se renderiza con PDF.js dentro de un <div> (un canvas por pagina), en lugar "
        "del visor embebido del navegador, que muchos moviles no cargaban dentro de un iframe.",
        "Se mantienen los botones Maximizar y Descargar / Imprimir.",
        "El servidor sandbox anade Access-Control-Allow-Origin: * a /ficha/*.pdf para que el visor "
        "(origen de la app) pueda leer el fichero por fetch cross-origin. El PDF es publico.",
        "PDF.js es self-hosted (sin CDN), build legacy (navegadores antiguos) y se carga bajo "
        "demanda (solo al abrir una ficha PDF).",
    ])
    pdf.section("Por que")
    pdf.body(
        "iOS Safari y Android Chrome, entre otros, no renderizan un PDF embebido en un <iframe> "
        "(ni <embed>/<object>): lo dejaban en blanco, aunque la descarga funcionaba. Renderizarlo "
        "con JavaScript (PDF.js) sobre un canvas funciona en casi todos los navegadores."
    )

    pdf.chapter_title("1", "Despliegue")
    pdf.body(
        "Sin migracion ni cambios de esquema/contrato. Basta con actualizar el codigo (frontend "
        "reconstruido + la cabecera CORS del sandbox) con el procedimiento de actualizacion "
        "habitual."
    )
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.22.1.pdf")
    build_pdf(output)
