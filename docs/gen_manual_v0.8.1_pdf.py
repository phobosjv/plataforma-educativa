# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.1.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.1", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.1", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.1. "
            "Esta version anade la posibilidad de MAXIMIZAR el contenedor de un ejercicio "
            "interactivo: ocupa casi toda la pantalla, con una barra superior fina que muestra el "
            "path del ejercicio (ciclo / curso / asignatura / titulo) y un boton para minimizar.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.1", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.1", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: maximizar el contenedor de ejercicios interactivos")
    pdf.bullet([
        "Nuevo boton 'Maximizar' en la esquina superior derecha del contenedor del ejercicio.",
        "Al maximizar, el ejercicio ocupa casi toda la pantalla (position: fixed), dejando arriba "
        "una barra fina con el path del ejercicio y el boton 'Minimizar'.",
        "El path se construye desde las taxonomias: ciclo / curso / asignatura / titulo "
        "(p. ej. primaria / 3o / conocimiento del medio / mapa espana).",
        ...
        "Se puede salir con la tecla Escape; el scroll del fondo se bloquea mientras esta maximizado.",
    ])
    pdf.section("Seguridad y estilo")
    pdf.bullet([
        "Se mantiene el aislamiento del sandbox en ambos modos: sandbox='allow-scripts' SIN "
        "allow-same-origin y servido desde el origen sandbox.",
        "Las taxonomias del path reutilizan la cache de React Query del catalogo (sin peticiones "
        "extra en la navegacion habitual). Clases CSS todas prefijadas cms-.",
    ])
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.1)", "Maximizar el contenedor de ejercicios interactivos (con path + minimizar)."),
        ("Hecho (V-0.8.0)", "Estampados de fondo desordenados + imagenes con formato en articulos."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.1.pdf")
    build_pdf(output)
