# Manual Técnico y de Usuario — Plataforma Educativa V-0.10.2
...
## 0. Novedades de V-0.10.2

**Descarga de copias de seguridad desde el navegador.**

- **Añadido:** nuevo endpoint `GET /api/v1/admin/backups/{nombre}` (solo `admin`) que sirve el
  fichero de la copia como adjunto, y botón **«Descargar»** en cada fila del panel «Copias de
  seguridad». La descarga usa el cliente tipado con el token de sesión (`parseAs: blob`), de modo
  que el administrador guarda la copia en su PC.
- **Seguridad:** el nombre solicitado se valida en el servicio de backup (formato exacto
  `app-YYYYMMDD-HHMMSS.sqlite3` + comprobación de contención en el directorio de copias); cualquier
  intento de *path traversal* (`../`, rutas absolutas) se rechaza con 404.
- **Sin cambios de esquema** → no hay migración. 136 tests en verde (13 nuevos). API → `0.10.2`.

### Versiones recientes
- **V-0.10.1** — Seguridad: endurecimiento de imágenes Docker (base bookworm + parches del SO).
- **V-0.10.0** — Robustez: backups automáticos de la BD + purga programada de papelera.
---
