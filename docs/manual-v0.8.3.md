# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.3
...
## 0. Novedades de V-0.8.3

Esta versión ajusta el **despliegue Docker** para un servidor de pruebas.

- **Puertos publicados** (`docker-compose.yml`): **frontend 5173 · api 5070 · sandbox 5071**
  (antes 5173 · 8000 · 8080). Los puertos internos no cambian (api 8000, sandbox 80).
- **Frontend:** Vite proxya `/api` y `/media` al servicio `api` por la red interna de Docker
...
- **`.dockerignore`** en `frontend/` y `backend/`: builds limpias (sin `node_modules` del host) y sin
  hornear secretos/BD en la imagen.
- **`.env.example`** con los valores del servidor (`SANDBOX_BASE_URL` / `APP_ORIGINS` /
  `CORS_ALLOW_ORIGINS` → `http://TU_SERVIDOR:<puerto>`). Deben usar la IP/dominio real (no
  `localhost`): el navegador carga los ejercicios desde el sandbox directamente.

### Versiones recientes
- **V-0.8.2** — Path navegable en el ejercicio maximizado + título del sitio como enlace.
- **V-0.8.1** — Maximizar el contenedor de ejercicios interactivos (path + minimizar).
---
