# Manual Técnico y de Usuario — Plataforma Educativa V-0.6.0
...
## 0. Novedades de V-0.6.0 (nueva funcionalidad)

Esta versión **cierra el tipo de contenido `interactivo`**: ya se pueden subir ejercicios HTML y
ejecutarlos de forma **aislada**.

- Un editor/admin **crea un contenido interactivo y sube su fichero HTML autocontenido** desde la
  pantalla de edición (`POST /api/v1/contenidos/{id}/html`, multipart, límite 2 MB).
...
- **Servidor sandbox:** nueva app ASGI independiente `app/sandbox.py` para desarrollo
  (`uvicorn app.sandbox:sandbox_app --port 8002`) y `nginx/sandbox.conf` para producción
  (`sandbox.<dominio>`), ambos con la **misma CSP** (a mantener sincronizada).
- **Backend:** invariante de dominio `Contenido.adjuntar_html_interactivo`, caso de uso
  `SubirHtmlContenidoHandler` (almacena por hash SHA-256, sin sanear, crea versión inmutable),
  `ContenidoResponse` expone `sandbox_url`. **No requiere migración** (la columna `hash_html` ya existía).
- **Frontend:** selector de tipo al crear, sección de subida del fichero al editar un interactivo, y
  render público del ejercicio en iframe usando `sandbox_url`.
- **Tests:** +14 (dominio, handler, endpoint de subida y seguridad del sandbox). Suite total:
  **97 tests**, todos en verde.

### Versiones recientes
- **V-0.5.0** — Fondos / estampados temáticos del sitio (6 temáticas + Ninguno).
- **V-0.4.0** — Creación/edición de artículos de texto (editor Tiptap, sanitización nh3 + DOMPurify).
---
