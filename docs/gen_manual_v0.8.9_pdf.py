# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.9.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.9", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.9", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.9. "
            "Esta version (robustez) sirve el frontend en Docker como BUILD ESTATICO tras nginx, en "
            "vez del servidor de desarrollo de Vite (no apto para produccion). nginx proxya /api y "
            "/media al backend y amplia el limite de subida a 16 MB.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.9", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.9", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Cambiado (robustez): frontend de produccion con nginx")
    pdf.bullet([
        "El frontend en Docker se sirve como BUILD ESTATICO tras nginx (Dockerfile multi-stage), "
        "en vez del servidor de desarrollo de Vite (npm run dev), no apto para produccion.",
        "Nuevo nginx/frontend.conf: sirve la SPA con fallback a index.html (React Router), proxya "
        "/api y /media al servicio api, cachea los assets con hash y sube client_max_body_size a 16M.",
        "client_max_body_size 16M evita el error 413 al subir imagenes (hasta 5 MB) y HTML; el "
        "default de nginx es 1 MB.",
        "docker-compose: frontend pasa a 5173:80 y deja de necesitar VITE_API_TARGET/VITE_ALLOWED_HOSTS.",
        "ALLOWED_HOSTS ya no hace falta: nginx sirve cualquier Host (IP y dominio funcionan solos).",
    ])
    pdf.section("Despliegue")
    pdf.bullet([
        "Aplicar con: docker compose up -d --build frontend.",
        "El servidor de desarrollo de Vite para uso local sigue igual (npm run dev).",
    ])
    ...
    pdf.kv_table([
        ...
        ("Hecho (V-0.8.8)", "Enlace 'Ver la web' en el panel admin (volver al catalogo publico)."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.9.pdf")
    build_pdf(output)
