# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.0. "
            "Esta version anade estampados de fondo DESORDENADOS configurables (sin dos iconos iguales "
            "adyacentes) e IMAGENES con formato (alineacion, tamano, pie) en el editor de "
            "articulos, con subida segura al nuevo contexto media.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: estampados de fondo desordenados (configurables)")
    pdf.bullet([
        "Nueva opcion Disposicion del estampado: Ordenada / Desordenada en Apariencia y ajustes.",
        "En Desordenada el patron se genera con posicion, rotacion y escala variables, y DOS "
        "iconos iguales NUNCA quedan adyacentes (vecindad toroidal; respaldo por backtracking).",
        "Backend: campo fondo_estilo en ConfiguracionSitio, migracion Alembic 007, validado.",
    ])
    pdf.section("Anadido: imagenes con formato en el editor de articulos")
    pdf.bullet([
        "Subir imagenes (PNG/JPG/GIF/WebP, max 5 MB) con alineacion, tamano y pie (figcaption).",
        "Se guardan como figure+img+figcaption (sanea nh3 servidor + DOMPurify cliente).",
        "Nuevo contexto media: POST /api/v1/media/imagenes (content-addressed). SVG rechazado.",
        "Imagenes servidas desde el origen de la app con X-Content-Type-Options: nosniff.",
    ])
    pdf.section("Backend y tests")
    pdf.bullet([
        "7 tests nuevos (fondo_estilo y subida de imagenes). Suite total: 111 tests, en verde.",
    ])
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.8.0.zip && cd plataforma-educativa-v0.8.0\n\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.0)", "Ejercicios interactivos: subida de HTML + origen sandbox aislado."),
        ("Hecho (V-0.8.0)", "Editar taxonomia (ciclo/curso/asignatura) de un contenido existente."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.0.pdf")
    build_pdf(output)
