# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.5.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.5", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.5", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.5. "
            "Esta version permite servir la web por IP Y por dominio a la vez: hosts permitidos "
            "(Vite server.allowedHosts) configurables por entorno (VITE_ALLOWED_HOSTS / ALLOWED_HOSTS), "
            "y orquestados desde el .env y docker-compose.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.5", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.5", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Corregido: Vite bloqueaba el acceso por dominio")
    pdf.bullet([
        "Sintoma: 'Blocked request. This host (...) is not allowed' al abrir la web por un dominio.",
        "Causa: Vite permite las IP por defecto, pero exige declarar los dominios en "
        "server.allowedHosts.",
    ])
    pdf.section("Anadido: hosts permitidos configurables (IP y dominio a la vez)")
    pdf.bullet([
        "vite.config.ts lee VITE_ALLOWED_HOSTS (lista separada por comas, o 'true' para cualquiera) "
        "y lo aplica a server.allowedHosts.",
        "Se propaga desde la variable ALLOWED_HOSTS del .env via docker-compose.yml.",
        ".env.example: guia de acceso dual. APP_ORIGINS admite varios origenes (separados por "
        "espacios) y CORS_ALLOW_ORIGINS varios (separados por comas).",
        "Nota: cambiar vite.config.ts requiere reconstruir la imagen del frontend (up -d --build).",
    ])
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.5)", "Acceso por IP y dominio a la vez (Vite allowedHosts configurable)."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.5.pdf")
    build_pdf(output)
