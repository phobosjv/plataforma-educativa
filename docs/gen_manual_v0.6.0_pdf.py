# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.6.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.6.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.6.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.6.0. "
            "Esta version cierra el tipo de contenido INTERACTIVO: subida del fichero HTML del "
            "ejercicio y ejecucion aislada en un iframe sandbox servido desde un ORIGEN "
            "distinto al de la app, con CSP estricta (CLAUDE.md seccion 10).",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.6.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.6.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: ejercicios interactivos (subida de HTML + sandbox)")
    pdf.bullet([
        "Un editor/admin puede crear un contenido de tipo interactivo y subir su fichero HTML "
        "autocontenido desde la pantalla de edicion.",
        "El HTML del ejercicio NO se sanea (debe ejecutar su JS) y se sirve desde un ORIGEN "
        "sandbox distinto al de la app, en un iframe sandbox=allow-scripts SIN allow-same-origin, "
        "con CSP estricta (default-src none; connect-src none; frame-ancestors <origenes app>).",
        "El render publico del ejercicio usa la URL sandbox_url devuelta por la API.",
    ])
    pdf.section("Servidor sandbox (dev y prod)")
    pdf.bullet([
        "Dev: app ASGI independiente app/sandbox.py (uvicorn app.sandbox:sandbox_app --port 8002).",
        "Prod: nginx/sandbox.conf sirviendo sandbox.<dominio> con la MISMA CSP (a sincronizar).",
        "Ruta canonica del ejercicio: GET /ejercicio/{hash} (hash SHA-256 content-addressed).",
    ])
    pdf.section("Backend y tests")
    pdf.bullet([
        "Dominio: invariante Contenido.adjuntar_html_interactivo (solo tipo interactivo).",
        "Caso de uso SubirHtmlContenidoHandler (almacena por hash, sin sanear, crea version nueva).",
        "Endpoint POST /api/v1/contenidos/{id}/html (multipart, editor/admin, limite 2 MB).",
        "ContenidoResponse expone sandbox_url. NO requiere migracion (hash_html ya existia).",
        "14 tests nuevos (dominio, handler, endpoint, seguridad sandbox). Suite total: 97, en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.6.0)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.6.0.zip && cd plataforma-educativa-v0.6.0\n\n"
    ...
    pdf.body(
        "Nota V-0.6.0: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.kv_table([
        ...
        ("Total", "97 tests - todos pasan en V-0.6.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.6.0)")
    pdf.body(
    ...
    pdf.kv_table([
        ("Hecho (V-0.6.0)", "Ejercicios interactivos: subida de HTML + origen sandbox aislado."),
        ("Siguiente", "Editar taxonomia (ciclo/curso/asignatura) de un contenido existente."),
        ("Siguiente", "Logotipo del sitio configurable. Buscador full-text (FTS5)."),
        ("V-1.0.0", "Despliegue en VPS con Nginx + HTTPS. E2E Playwright. Contexto media."),
    ])
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.6.0.pdf")
    build_pdf(output)
