# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.4.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.4", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.4", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.4. "
            "Esta version corrige el bucle de reinicio de la API en Docker "
            "(ModuleNotFoundError: app) mediante PYTHONPATH=/app, y mejora los mensajes de error de "
            "login (distingue credenciales incorrectas de servidor no disponible).",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.4", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.4", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Corregido: API en bucle de reinicio en Docker")
    pdf.bullet([
        "Sintoma: el contenedor api se reiniciaba en bucle con ModuleNotFoundError: No module "
        "named 'app' al ejecutar Alembic en el entrypoint.",
        "Causa: el layout plano del backend no instala el paquete app en site-packages con "
        "pip install ., y el comando alembic no anade /app al sys.path.",
        "Solucion: PYTHONPATH=/app en backend/Dockerfile (builds limpias) y en el servicio api "
        "del docker-compose.yml (permite aplicar el arreglo sin reconstruir la imagen).",
    ])
    pdf.section("Cambiado: mensajes de error de login mas claros")
    pdf.bullet([
        "El formulario de acceso distingue: credenciales incorrectas (401), servidor no "
        "disponible (5xx, p. ej. API caida o reiniciandose) y fallo de conexion (sin respuesta).",
        "Antes mostraba siempre 'Credenciales incorrectas', lo que ocultaba caidas del servidor.",
    ])
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.4)", "Fix API en bucle de reinicio (PYTHONPATH) + errores de login claros."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.4.pdf")
    build_pdf(output)
