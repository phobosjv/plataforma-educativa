# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.3.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.3.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.3.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.3.0. "
            "Esta version anade los AJUSTES GENERALES del sitio: nombre editable y seleccion de "
            "fuente de letra (catalogo curado y self-hosted para no exponer datos de menores a "
            "terceros), aplicables en tiempo real desde el panel de administracion.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.3.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.3.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: ajustes generales del sitio")
    pdf.bullet([
        "El NOMBRE del sitio es editable desde el panel 'Apariencia y ajustes' y se refleja en la "
        "barra publica, el pie de pagina y el sidebar de administracion.",
        "Selector de FUENTE de letra con catalogo curado para portal infantil/primaria: "
        "Nunito y Quicksand (amigables); Lexend, Atkinson Hyperlegible y Andika (alta "
        "legibilidad / lectores principiantes); Sistema (sin descarga) por defecto.",
        "La fuente se aplica en tiempo real a todo el sitio (variable CSS --cms-font).",
    ])
    pdf.section("Privacidad: fuentes self-hosted")
    pdf.bullet([
        "Las fuentes se sirven desde la PROPIA app (subset latino, pesos 400/700), no desde un "
        "CDN externo: no se expone la IP de los menores a terceros (RGPD/DSA, CLAUDE.md s10).",
        "Los .woff2 viven en frontend/public/fonts/ y los descarga el script "
        "frontend/scripts/download_fonts.py (ejecucion unica).",
    ])
    pdf.section("Backend y tests")
    pdf.bullet([
        "Dominio: campo fuente_activa y metodos cambiar_nombre / cambiar_fuente con validacion "
        "(nombre no vacio <= 80 chars; fuente dentro de FUENTES_PERMITIDAS).",
        "Endpoint PUT /api/v1/config/general (solo admin). Migracion Alembic 005 (columna "
        "fuente_activa en site_config). Cliente OpenAPI regenerado.",
        "4 tests de integracion nuevos. Suite total: 78 tests, todos en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.3.0)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.3.0.zip && cd plataforma-educativa-v0.3.0\n\n"
    ...
    pdf.endpoint_table([
        ("GET", "/config/", "-", "Obtener config: nombre_sitio, fuente_activa, paleta, paletas"),
    ...
    pdf.body(
        "Nota V-0.3.0: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.kv_table([
        ...
        ("Total", "78 tests - todos pasan en V-0.3.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.3.0)")
    pdf.body(
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.3.0.pdf")
    build_pdf(output)
