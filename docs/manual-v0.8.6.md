# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.6
...
## 0. Novedades de V-0.8.6

Corrección del **403 Forbidden** al abrir ejercicios interactivos en Docker.

- **Corregido:** el HTML del ejercicio se guardaba con permisos `0o600` (herencia de
  `tempfile.mkstemp`), legible solo por el usuario de la API (root). El nginx del sandbox lo sirve
  como usuario `nginx`, que no podía leerlo → **403 Forbidden**. Ahora se guarda como **`0o644`**
  (contenido público servido de forma aislada). En local no se reproducía porque el sandbox Python
  corría con el mismo usuario.
