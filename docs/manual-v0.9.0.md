# Manual Técnico y de Usuario — Plataforma Educativa V-0.9.0
...
## 0. Novedades de V-0.9.0

Producción: **HTTPS automático** con un proxy inverso **Caddy**.

- **Añadido:** nuevo servicio **Caddy** (`caddy/Caddyfile`), **único** servicio expuesto (80/443),
  con **certificados automáticos de Let's Encrypt** (auto-renovación) y **HSTS**.
...
- **Cambiado:** `frontend`, `api` y `sandbox` pasan a ser **internos** (ya no publican puertos).
  `SANDBOX_BASE_URL`/`APP_ORIGINS`/`CORS_ALLOW_ORIGINS` se **derivan** de `APP_DOMAIN`/`SANDBOX_DOMAIN`
  en `docker-compose.yml`. `.env.example` simplificado al modelo de dominios.
- Volúmenes `caddy_data`/`caddy_config` para **persistir los certificados**.
- **Despliegue:** DNS A de ambos dominios → IP del servidor, puertos **80/443** redirigidos,
  rellenar `.env` y `docker compose up -d --build`. El acceso pasa a ser **solo por `https://APP_DOMAIN`**.
