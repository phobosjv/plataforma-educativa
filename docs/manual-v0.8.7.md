# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.7
...
## 0. Novedades de V-0.8.7

Corrección: el **sandbox cacheaba los errores** (403/404) durante un año.

- **Corregido:** la cabecera `Cache-Control: public, max-age=31536000, immutable` se enviaba con
  `always`, por lo que también cacheaba como **inmutable** las respuestas de error. Tras corregir un
  ejercicio que daba 403, el navegador seguía mostrando el 403 cacheado sin reintentar (ni aparecía
  petición en el log del sandbox). Ahora ese `Cache-Control` se aplica **sin `always`** (solo a
  respuestas 2xx/3xx) en `nginx/sandbox.conf.template` y `nginx/sandbox.conf`. El sandbox Python ya
  solo lo ponía en el 200.
- **Cómo aplicarlo:** actualizar la plantilla y `docker compose restart sandbox`; y **vaciar la caché
  del navegador** (incógnito o recarga forzada) para soltar un error ya cacheado.

### Versiones recientes
- **V-0.8.6** — Fix 403 Forbidden en ejercicios interactivos (permisos `0o644`).
- **V-0.8.5** — Acceso por IP y dominio a la vez (Vite `allowedHosts` configurable).
---
...
### 3.6 Origen sandbox (servidor aparte)
...
### 5.2 Arranque rápido con Docker (servidor de pruebas)

Puertos publicados: **frontend 5173 · api 5070 · sandbox 5071**.
...
cp .env.example .env
# Editar .env: sustituir TU_SERVIDOR por la IP/dominio real y poner SECRET_KEY:
#   SANDBOX_BASE_URL=http://TU_SERVIDOR:5071
#   APP_ORIGINS=http://TU_SERVIDOR:5173
#   CORS_ALLOW_ORIGINS=http://TU_SERVIDOR:5173

# 3. Levantar servicios (construye las imágenes)
docker compose up -d --build

# 4. Verificar que todo funciona
curl http://TU_SERVIDOR:5070/api/v1/config/
...
> (no `localhost`): el navegador carga los ejercicios interactivos desde el sandbox (5071)
> directamente, y el `frame-ancestors` del sandbox debe permitir el origen del frontend (5173).
