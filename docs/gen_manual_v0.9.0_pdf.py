# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.9.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.9.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.9.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.9.0. "
            "Esta version anade HTTPS de produccion: un proxy inverso Caddy con certificados "
            "automaticos de Let's Encrypt. La web y el sandbox (en su propio subdominio) se sirven "
            "por https; frontend, api y sandbox quedan internos y solo Caddy se expone (80/443).",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.9.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.9.0", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: HTTPS de produccion con Caddy (Let's Encrypt)")
    pdf.bullet([
        "Nuevo proxy inverso Caddy (caddy/Caddyfile): UNICO servicio expuesto (80/443), con "
        "certificados automaticos de Let's Encrypt y auto-renovacion, mas HSTS.",
        ...
        "frontend, api y sandbox pasan a ser INTERNOS (ya no publican puertos).",
        "SANDBOX_BASE_URL/APP_ORIGINS/CORS se derivan de APP_DOMAIN/SANDBOX_DOMAIN en el compose.",
        "Volumenes caddy_data/caddy_config para persistir los certificados (rate limit de LE).",
    ])
    pdf.section("Despliegue")
    pdf.bullet([
        "1) DNS: registros A de APP_DOMAIN y SANDBOX_DOMAIN -> IP del servidor.",
        "2) Router: redirigir 80 y 443 al servidor.",
        "3) .env: APP_DOMAIN, SANDBOX_DOMAIN, ACME_EMAIL, SECRET_KEY, admin.",
        "4) docker compose up -d --build. El acceso pasa a ser solo por https://APP_DOMAIN.",
    ])
    ...
    pdf.kv_table([
        ...
        ("Hecho (V-0.8.9)", "Robustez: frontend de produccion (build estatico tras nginx)."),
        ("Siguiente", "Imagen base backend sin vulns. Backups BD + purga programada. E2E Playwright."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.9.0.pdf")
    build_pdf(output)
