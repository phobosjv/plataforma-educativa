# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.4.0.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.4.0", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.4.0", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.4.0. "
            "Esta version anade la CREACION Y EDICION de articulos de texto desde el panel: "
            "formulario con editor WYSIWYG (Tiptap) y sanitizacion del HTML en servidor (nh3) y "
            "cliente (DOMPurify), segun la sanitizacion asimetrica exigida para menores.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.4.0", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.4.0 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: creacion y edicion de articulos de texto")
    pdf.bullet([
        "Formulario de contenidos (/admin/contenidos/nuevo y /admin/contenidos/{id}/editar) con "
        "titulo, descripcion, etiquetas, clasificacion (ciclo/curso/asignatura) y cuerpo.",
        "Editor WYSIWYG (Tiptap): negrita, cursiva, tachado, encabezados, listas, cita y enlaces.",
        "Botones '+ Nuevo articulo' y 'Editar' en la lista de contenidos.",
    ])
    pdf.section("Seguridad: sanitizacion del HTML de articulos (s10)")
    pdf.bullet([
        "Servidor (SIEMPRE): puerto de dominio HtmlSanitizer + impl Nh3HtmlSanitizer (nh3), con "
        "allowlist conservadora (sin script/style/iframe, sin atributos on*, sin javascript:).",
        "Cliente: segunda capa con DOMPurify al renderizar el articulo en la vista publica.",
        "Sanitizacion asimetrica: el HTML de ejercicios interactivos NO se sanea (va aislado).",
    ])
    pdf.section("Alcance y tests")
    pdf.bullet([
        "Cubre solo el tipo texto. Los interactivos (subida de HTML) iran con el subdominio sandbox.",
        "La clasificacion se asigna al crear (el contrato actual no la edita en actualizacion).",
        "2 tests de integracion de sanitizacion nuevos. Suite total: 80 tests, todos en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.4.0)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.4.0.zip && cd plataforma-educativa-v0.4.0\n\n"
    ...
    pdf.body(
        "Nota V-0.4.0: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.bullet([
        ...
        "Pendiente: sanitizacion en cliente con DOMPurify (V-0.4.0).",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "80 tests - todos pasan en V-0.4.0"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.4.0)")
    pdf.body(
    ...
    pdf.kv_table([
        ("V-0.4.0", "Formulario completo de creacion/edicion de contenidos. "
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.4.0.pdf")
    build_pdf(output)
