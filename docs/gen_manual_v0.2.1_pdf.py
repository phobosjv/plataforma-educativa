# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.2.1.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.2.1", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.2.1", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.2.1. "
            "Esta version es un PATCH de correccion: arregla el error 500 al aplicar o guardar "
            "paletas de color (contexto configuration), mejora el manejo de errores en el "
            "frontend y elimina los problemas de CORS en desarrollo mediante proxy de Vite.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.2.1", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.2.1 (release de correccion)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Corregido")
    pdf.bullet([
        "Error 500 al aplicar o guardar paletas de color (contexto configuration). Cuando la "
        "fila singleton site_config aun no existia, el repositorio insertaba dos filas con la "
        "misma clave primaria (IntegrityError: UNIQUE constraint failed: site_config.id).",
        "Causa raiz: SessionLocal usa autoflush=False, asi que session.get() no encontraba el "
        "modelo recien anadido (pendiente de flush) y save() creaba un duplicado.",
        "Solucion: logica de singleton unificada en _get_or_create_model() con flush() explicito; "
        "el modelo pasa a persistent y get/save operan sobre la MISMA instancia.",
    ])
    pdf.section("Cambiado (frontend)")
    pdf.bullet([
        "Las mutaciones de paletas propagan los errores de la API en vez de tragarselos; la "
        "pagina de Apariencia muestra un banner de error accesible (role=alert).",
        "El cliente de API usa baseUrl relativo y Vite reenvia /api al backend mediante proxy: "
        "sin problemas de CORS en desarrollo. Puerto 5173 fijo (strictPort).",
    ])
    pdf.section("Anadido (tests)")
    pdf.bullet([
        "tests/integration/test_configuration_endpoints.py (7 tests).",
        "El fixture replica autoflush=False de produccion para que el caso de regresion sea "
        "valido (con autoflush=True el bug no se reproduce).",
    ...
    pdf.kv_table([
        ...
        ("content", "CRUD contenidos, versionado inmutable, papelera, ciclo de vida"),
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.2.1)")
    pdf.body(
    ...
    pdf.bullet([
        ...
        "Puerto 8000 accesible (o 80/443 con proxy Nginx/Caddy)",
        "Subdominio sandbox.<dominio> para ejercicios interactivos",
    ])
    pdf.section("Arranque rapido con Docker")
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.2.1.zip && cd plataforma-educativa-v0.2.1\n\n"
        "# 2. Configurar variables de entorno\n"
        "cp backend/.env.example backend/.env\n"
        "# Editar: SECRET_KEY, DEFAULT_ADMIN_PASSWORD\n\n"
        "# 3. Levantar\n"
        "docker compose up -d\n\n"
        "# El entrypoint ejecuta automaticamente:\n"
        "#   alembic upgrade head\n"
        "#   Si no hay usuarios -> crea admin con credenciales del .env"
    )
    ...
    pdf.section("Contenidos (/contents/)")
    pdf.endpoint_table([
        ("GET", "/contents/", "-", "Listar contenidos publicados (catalogo publico)"),
        ("GET", "/contents/{id}", "-", "Obtener contenido por ID"),
        ("GET", "/contents/all", "editor+", "Listar todos (incluye borradores)"),
        ("GET", "/contents/trash", "editor+", "Listar papelera"),
        ("POST", "/contents/", "editor+", "Crear contenido (borrador)"),
        ...
        ("DELETE", "/contents/{id}", "editor+", "Mover a papelera (soft-delete)"),
    ...
    pdf.endpoint_table([
        ("GET", "/config/", "-", "Obtener config: nombre_sitio, paleta_activa, paletas custom"),
    ...
    pdf.body(
        "Nota V-0.2.1: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.bullet([
        ...
        "Servidos desde sandbox.<dominio> con CSP estricta.",
    ...
    pdf.bullet([
        ...
        "Pendiente: sanitizacion en cliente con DOMPurify (V-0.3.0).",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "74 tests - todos pasan en V-0.2.1"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Ejercicio interactivo", "Se ejecuta en el navegador dentro de un iframe. "
         "Sin acceso a datos del usuario ni del sitio. Seguro para menores."),
    ...
    pdf.body(
        "NOTA: El formulario de creacion y edicion completa de contenidos esta "
        "planificado para V-0.3.0."
    )
    ...
    # 14. Apariencia
    pdf.chapter_title("14", "Panel admin - Apariencia (paletas de color)")
    ...
    pdf.section("Manejo de errores (mejora de V-0.2.1)")
    pdf.body(
    ...
    pdf.kv_table([
        ("V-0.3.0", "Formulario completo de creacion/edicion de contenidos. "
         "Subida de HTML para ejercicios. Editor de texto enriquecido (Tiptap). "
         "Sanitizacion en cliente con DOMPurify."),
        ("V-0.4.0", "Buscador full-text (FTS5) en catalogo publico. "
         "Paginacion. Soporte de etiquetas/tags."),
        ("V-0.5.0", "Contexto media (imagenes y ficheros adjuntos)."),
        ("V-1.0.0", "Despliegue en VPS con Nginx + HTTPS. E2E con Playwright. "
         "Configuracion de nombre del sitio y logotipo."),
    ])
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.2.1.pdf")
    build_pdf(output)
