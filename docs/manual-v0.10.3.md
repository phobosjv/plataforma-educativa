# Manual Técnico y de Usuario — Plataforma Educativa V-0.10.3
...
## 0. Novedades de V-0.10.3

**Exportación completa del sitio (BD + media) y copia incremental de media.**

- **Exportación completa descargable:** nuevo endpoint `POST /api/v1/admin/export` (solo `admin`) y
  botón **«Exportar todo (BD + media)»** en el panel. Genera un `.tar.gz` con `data/app.sqlite3`
  (copia en caliente), la carpeta `media/` entera y un `manifest.json`. Con ese archivo se puede
  **migrar el servidor** o **recuperar el sitio entero** ante un fallo total. **No incluye el `.env`**
  (secretos): se gestiona aparte.
- **Copia incremental automática de `media`:** en cada ciclo de backup, junto a la copia de la BD, se
  sincroniza un espejo de `media` en `./data/backups/media/` copiando solo los ficheros nuevos
  (content-addressed e inmutables). Desactivable con `MEDIA_BACKUP_ENABLED=false`.
- **Restauración/migración:** sin endpoint de importación (sobrescribir la BD viva es arriesgado); se
  hace en el servidor con el stack parado. Ver `docs/copias-y-restauracion.md`.
- **Sin cambios de esquema** → no hay migración. 144 tests en verde (8 nuevos). API → `0.10.3`.

### Versiones recientes
- **V-0.10.2** — Descarga de copias de seguridad desde el navegador.
- **V-0.10.1** — Seguridad: endurecimiento de imágenes Docker (base bookworm + parches del SO).
---
