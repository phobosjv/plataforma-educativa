# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.0
...
## 0. Novedades de V-0.8.0

Esta versión añade **estampados de fondo desordenados** configurables e **imágenes con formato** en
el editor de artículos.

- **Fondos desordenados:** nueva opción **Disposición: Ordenada / Desordenada** en *Apariencia y
  ajustes*. En "Desordenada" el patrón se genera con posición, rotación y escala variables y **dos
  iconos iguales nunca quedan adyacentes** (vecindad toroidal, con respaldo determinista). Backend:
  campo `fondo_estilo`, migración Alembic **007**, validado en `PUT /config/general`.
- **Imágenes en artículos:** subir imágenes (PNG/JPG/GIF/WebP, máx. 5 MB) con **alineación**
  (izquierda/centro/derecha), **tamaño** (S/M/L/100%) y **pie** (`figcaption`). Se guardan como
  `<figure>`+`<img>`+`<figcaption>` y se sanean en servidor (nh3) y cliente (DOMPurify).
- **Contexto `media`:** `POST /api/v1/media/imagenes` (multipart, editor/admin) almacena la imagen
  content-addressed y devuelve su URL; **SVG rechazado** (vector XSS). Se sirven desde el origen de
  la app con `X-Content-Type-Options: nosniff`.
- **Tests:** +7. Suite total: **111 tests**, todos en verde.

### Versiones recientes
- **V-0.7.0** — Catálogo navegable por taxonomías + borrado definitivo + botón de interactivos.
