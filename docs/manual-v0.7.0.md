# Manual Técnico y de Usuario — Plataforma Educativa V-0.7.0
...
## 0. Novedades de V-0.7.0

Esta versión hace el **catálogo público navegable por taxonomías** (pensado para niños), añade la
**eliminación definitiva** desde la papelera y un **botón claro para crear ejercicios interactivos**.

- **Catálogo navegable:** navegación guiada por pasos — **¿En qué curso estás?** → **elige
  asignatura** → **ejercicios**. Tarjetas grandes y coloridas (cursos agrupados por ciclo con nº de
  actividades; asignaturas con su color), **migas de pan** grandes y clicables, y estado en la URL
  (`?curso=&asignatura=`) para que el botón "atrás" funcione y sea compartible. Solo se muestran
  cursos/asignaturas con actividades (sin callejones vacíos); botón **"Ver todo el catálogo"**.
- **Eliminación definitiva:** botón **"Eliminar definitivamente"** (con confirmación) en la papelera;
  solo **admin**, irreversible. Endpoint `DELETE /api/v1/contenidos/{id}/purgar`; las versiones se
  borran en cascada.
- **Crear interactivos visible:** dos botones en *Contenidos* ("+ Artículo de texto" y "+ Ejercicio
  interactivo"); el formulario llega con el tipo preseleccionado.
- **Tests:** +3 de integración (purga). Suite total: **104 tests**, todos en verde.

### Versiones recientes
- **V-0.6.1** — Asignar/editar ciclo-curso-asignatura de un contenido (+ fix de persistencia).
- **V-0.6.0** — Ejercicios interactivos: subida de HTML + origen sandbox aislado (CSP).
---
