# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.6.1.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.6.1", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.6.1", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.6.1. "
            "Esta version permite asignar y editar la clasificacion (ciclo/curso/asignatura) de un "
            "contenido tanto al crear como al editar, y corrige que esa clasificacion no se "
            "persistia al guardar (bug del repositorio).",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.6.1", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.6.1 (nueva funcionalidad)", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: clasificacion (ciclo/curso/asignatura) editable")
    pdf.bullet([
        "Los selectores de ciclo, curso y asignatura aparecen ahora al CREAR y al EDITAR un "
        "contenido (antes solo al crear). Se puede reclasificar y desasignar (dejar en blanco).",
        "ContenidoResponse expone ciclo_id, curso_id y asignatura_id (antes eran write-only).",
    ])
    pdf.section("Corregido")
    pdf.bullet([
        "La clasificacion no se persistia al editar: SqlAlchemyContenidoRepository.save() no "
        "copiaba ciclo_id/curso_id/asignatura_id al modelo. El PUT se reflejaba en la respuesta "
        "pero NO se guardaba en la BD. Ahora se persiste correctamente.",
    ])
    pdf.section("Backend y tests")
    pdf.bullet([
        "PUT /api/v1/contenidos/{id} acepta ciclo_id/curso_id/asignatura_id. Solo reasigna si el "
        "cliente envia esos campos (model_fields_set): un PUT parcial no borra; null desasigna.",
        "4 tests de integracion nuevos (asignar, reasignar con persistencia, conservar, desasignar).",
        "Suite total: 101 tests, todos en verde.",
    ])
    ...
    pdf.section("Patron singleton de configuracion (corregido en V-0.6.1)")
    pdf.body(
    ...
    pdf.code(
        "# 1. Descomprimir\n"
        "unzip plataforma-educativa-v0.6.1.zip && cd plataforma-educativa-v0.6.1\n\n"
    ...
    pdf.body(
        "Nota V-0.6.1: estos endpoints crean la fila singleton de configuracion de forma "
    ...
    pdf.kv_table([
        ...
        ("Total", "101 tests - todos pasan en V-0.6.1"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.section("Manejo de errores (mejora de V-0.6.1)")
    pdf.body(
    ...
    pdf.kv_table([
        ("Hecho (V-0.6.1)", "Ejercicios interactivos: subida de HTML + origen sandbox aislado."),
        ("Hecho (V-0.6.1)", "Editar taxonomia (ciclo/curso/asignatura) de un contenido existente."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.6.1.pdf")
    build_pdf(output)
