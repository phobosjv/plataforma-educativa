# Manual Técnico y de Usuario — Plataforma Educativa V-0.6.1
...
## 0. Novedades de V-0.6.1

Esta versión permite **asignar y editar la clasificación** (ciclo / curso / asignatura) de un
contenido, y **corrige** que esa clasificación no se persistía al guardar.
...
- `ContenidoResponse` expone `ciclo_id`, `curso_id` y `asignatura_id` (antes eran *write-only*: se
  enviaban al crear pero el contrato nunca los devolvía, por lo que no se veían al editar).
- **Corregido:** `SqlAlchemyContenidoRepository.save()` no copiaba la taxonomía al modelo, así que un
  `PUT` con clasificación se reflejaba en la respuesta pero **no se guardaba** en la base de datos.
- `PUT /api/v1/contenidos/{id}` acepta `ciclo_id/curso_id/asignatura_id` con semántica segura
  (`model_fields_set`): un `PUT` parcial que los omite no borra la clasificación; enviar `null` la desasigna.
- **Tests:** +4 de integración (incluida verificación de persistencia vía GET). Suite total:
  **101 tests**, todos en verde.

### Versiones recientes
- **V-0.6.0** — Ejercicios interactivos: subida de HTML + origen sandbox aislado (CSP).
- **V-0.5.0** — Fondos / estampados temáticos del sitio.
---
