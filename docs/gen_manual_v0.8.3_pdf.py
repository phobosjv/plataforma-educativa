# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.8.3.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.8.3", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.8.3", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.8.3. "
            "Esta version ajusta el DESPLIEGUE DOCKER para un servidor de pruebas: puertos "
            "publicados frontend 5173, api 5070 y sandbox 5071, sandbox por plantilla nginx con "
            "frame-ancestors configurable, y .dockerignore para builds limpias.",
            border=0, fill=True,
        )
        self.ln(5)
        ...
        self.cell(0, 6, "Version del software: V-0.8.3", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.8.3", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Cambiado: despliegue Docker para servidor de pruebas")
    pdf.bullet([
        "Puertos PUBLICADOS en docker-compose.yml: frontend 5173, api 5070, sandbox 5071 "
        ...
        "(VITE_API_TARGET=http://api:8000); el cliente usa rutas relativas.",
        "Sandbox por plantilla nginx/sandbox.conf.template con frame-ancestors ${APP_ORIGINS}, "
        "renderizada por envsubst (NGINX_ENVSUBST_FILTER=APP_ORIGINS).",
        "restart: unless-stopped en los tres servicios.",
    ])
    pdf.section("Anadido: .dockerignore y .env.example de servidor")
    pdf.bullet([
        ".dockerignore en frontend/ (excluye node_modules del host: sus binarios nativos romperian "
        "la imagen Linux) y backend/ (excluye .env, la BD y cachas para no hornearlos en la imagen).",
        ".env.example con SANDBOX_BASE_URL / APP_ORIGINS / CORS_ALLOW_ORIGINS apuntando a "
        "http://TU_SERVIDOR:<puerto> y el admin inicial.",
        "IMPORTANTE: SANDBOX_BASE_URL y APP_ORIGINS deben usar la IP/dominio real del servidor "
        "(no localhost): el navegador carga los ejercicios desde el sandbox directamente.",
    ])
    ...
    pdf.kv_table([
        ("Hecho (V-0.8.3)", "Despliegue Docker para servidor de pruebas (puertos 5173/5070/5071)."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.8.3.pdf")
    build_pdf(output)
