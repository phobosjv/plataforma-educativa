# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.10.3.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.10.3", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.10.3", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.10.3. "
        ...
        self.cell(0, 6, "Version del software: V-0.10.3", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.10.3", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Anadido: exportacion completa del sitio (BD + media)")
    pdf.bullet([
        "Nuevo endpoint POST /api/v1/admin/export (solo admin) y boton 'Exportar todo (BD + media)' "
        "en el panel. Genera un .tar.gz con data/app.sqlite3 (copia en caliente), la carpeta media "
        "entera y un manifest.json.",
        "Con ese archivo se puede MIGRAR el servidor o RECUPERAR el sitio entero ante un fallo total.",
        "No incluye el .env (secretos): se gestiona aparte en el servidor de destino.",
    ])
    pdf.section("Anadido: copia incremental automatica de media")
    pdf.bullet([
        "En cada ciclo de backup, junto a la copia de la BD, se sincroniza un espejo de media en "
        "./data/backups/media copiando solo los ficheros nuevos (content-addressed, inmutables).",
        "Desactivable con MEDIA_BACKUP_ENABLED=false.",
    ])
    pdf.section("Restauracion / migracion")
    pdf.bullet([
        "No hay endpoint de importacion (sobrescribir la BD viva es arriesgado): la restauracion se "
        "hace en el servidor con el stack parado. Ver docs/copias-y-restauracion.md.",
        "La version de la app se centraliza en app/version.py (evita el drift de la version).",
        "Sin cambios de esquema: no hay migracion. 144 tests en verde (8 nuevos). API -> 0.10.3.",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "123 tests - todos pasan en V-0.10.3"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.10.3)", "Robustez: backups automaticos de la BD + purga programada de papelera."),
        ("Hecho (V-0.9.0)", "Produccion: HTTPS con Caddy (Let's Encrypt) + sandbox en subdominio."),
        ("Siguiente", "E2E Playwright (incl. test seguridad sandbox). Usuario no-root en el backend."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.10.3.pdf")
    build_pdf(output)
