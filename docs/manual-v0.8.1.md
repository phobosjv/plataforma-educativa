# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.1
...
## 0. Novedades de V-0.8.1

Esta versión añade la posibilidad de **maximizar el contenedor de un ejercicio interactivo**.

- **Maximizar ejercicio:** nuevo botón **"⤢ Maximizar"** en la esquina superior derecha del
  contenedor del ejercicio. Al pulsarlo, el ejercicio ocupa **casi toda la pantalla**
  (`position: fixed`), dejando arriba una **barra fina**.
- **Barra superior:** muestra el **path** del ejercicio construido desde las taxonomías
  (`ciclo / curso / asignatura / título`, p. ej. *primaria / 3º / conocimiento del medio / mapa
  españa*) y el botón **"⤡ Minimizar"**.
- **Sin recargas:** el **mismo iframe** se reutiliza en ambos modos, por lo que maximizar/minimizar
  **no recarga** el ejercicio (no se pierde el progreso del alumno).
- **UX:** se puede salir con la tecla **Escape** y el scroll del fondo se bloquea mientras está
  maximizado.
- **Seguridad:** se mantiene el aislamiento del sandbox en ambos modos (`sandbox="allow-scripts"`
  sin `allow-same-origin`, servido desde el origen sandbox). Clases CSS prefijadas `cms-`.

### Versiones recientes
- **V-0.8.0** — Estampados de fondo desordenados configurables + imágenes con formato en artículos.
