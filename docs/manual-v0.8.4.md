# Manual Técnico y de Usuario — Plataforma Educativa V-0.8.4
...
## 0. Novedades de V-0.8.4

Versión de corrección del despliegue Docker.

- **Corregido — API en bucle de reinicio en Docker:** el contenedor `api` se reiniciaba con
  `ModuleNotFoundError: No module named 'app'` al ejecutar Alembic. Causa: el layout plano no
  instala `app` en site-packages con `pip install .` y el comando `alembic` no añade `/app` al
  `sys.path`. Solución: **`PYTHONPATH=/app`** en `backend/Dockerfile` y en el servicio `api` del
  `docker-compose.yml` (este último permite aplicar el arreglo **sin reconstruir** la imagen).
- **Cambiado — mensajes de error de login:** el formulario distingue **credenciales incorrectas**
  (401), **servidor no disponible** (5xx, API caída/reiniciándose) y **fallo de conexión**, en vez
  de mostrar siempre "Credenciales incorrectas".

### Versiones recientes
- **V-0.8.3** — Despliegue Docker para servidor de pruebas (puertos 5173/5070/5071).
- **V-0.8.2** — Path navegable en el ejercicio maximizado + título del sitio como enlace.
---
