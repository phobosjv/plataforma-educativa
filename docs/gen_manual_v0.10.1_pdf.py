# -*- coding: utf-8 -*-
"""Genera docs/manual-v0.10.1.pdf usando fpdf2."""
...
class ManualPDF(FPDF):
    ...
    def header(self) -> None:
        ...
        self.cell(0, 8, "Plataforma Educativa - Manual Tecnico y de Usuario  V-0.10.1", align="L")
    ...
    def cover(self) -> None:
        ...
        self.set_y(58)
        self.cell(0, 10, "Manual Tecnico y de Usuario - V-0.10.1", align="C",
                  new_x="LMARGIN", new_y="NEXT")
        self.set_text_color(*BLACK)
        self.set_y(88)
        ...
        self.multi_cell(
            ...
            "guia de instalacion y manual de uso de la Plataforma Educativa V-0.10.1. "
        ...
        self.cell(0, 6, "Version del software: V-0.10.1", align="C", new_x="LMARGIN", new_y="NEXT")
...
def build_pdf(output_path: str) -> None:
    ...
    pdf.cell(0, 10, "NOVEDADES DE V-0.10.1", align="C",
             new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(*BLACK)
    pdf.ln(4)
    pdf.chapter_title("0", "Cambios de esta version")
    pdf.section("Seguridad de las imagenes Docker")
    pdf.bullet([
        "Backend: base python:3.12-slim -> python:3.12-slim-bookworm (release de Debian explicita) "
        "y parches de seguridad del SO en el build (apt-get upgrade), que aplican las correcciones "
        "de Debian posteriores a la imagen base (el grueso de los hallazgos del escaner).",
        "Se actualizan pip/setuptools/wheel (sus versiones antiguas son un hallazgo recurrente).",
        "Frontend: etapa final nginx:alpine con apk upgrade (paridad de parches del SO).",
        "Nueva guia docs/seguridad-imagenes.md: como escanear las imagenes ya construidas (Trivy, "
        "--ignore-unfixed), fijar la base por digest y endurecimiento pendiente (usuario no-root).",
    ])
    pdf.section("Notas")
    pdf.bullet([
        "El escaner del IDE marca el tag base ANTES del apt-get upgrade; tras el build, las CVEs con "
        "parche quedan resueltas y las restantes suelen ser 'sin fix' en Debian.",
        "Sin cambios de codigo, API ni esquema: 123 tests siguen en verde.",
        "Al desplegar no hace falta nada especial; el endurecimiento se aplica al reconstruir.",
    ])
    ...
    pdf.kv_table([
        ...
        ("Total", "123 tests - todos pasan en V-0.10.1"),
    ])
    pdf.code(
        "cd backend\n"
        "pytest -v                        # todos\n"
    ...
    pdf.kv_table([
        ("Hecho (V-0.10.1)", "Robustez: backups automaticos de la BD + purga programada de papelera."),
...
if __name__ == "__main__":
    output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manual-v0.10.1.pdf")
    build_pdf(output)
